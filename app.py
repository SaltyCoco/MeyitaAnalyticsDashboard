...
def pneumonia_deaths():
    ...
    pneumonia_deaths_fig = go.Figure(data=data, layout=layout)

    return pneumonia_deaths_fig

def influenza_deaths():
    df = pd.read_csv("data/cdc/National_Custom_Data.csv")
    df = df[["SEASON", "WEEK", "NUM PNEUMONIA DEATHS", "NUM INFLUENZA DEATHS"]]
    df = df.sort_values(by=["WEEK"], ascending=True)
    df2019_20 = df[df["SEASON"]=="2019-20"]
    ...
    influenza_deaths_fig = go.Figure(data=data, layout=fl_layout)

    return influenza_deaths_fig
...
flu_layout = html.Div(children=[
    ...
    html.Br(),
    dbc.Row([
        dbc.Col(dcc.Graph(figure=pneumonia_deaths())),
        dbc.Col(dcc.Graph(figure=influenza_deaths()))
    ]),
    html.Br(),
    html.Br(),
    html.Br(),
